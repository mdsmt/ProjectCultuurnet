			...
			try{ 
	            ...
	            if($result->num_rows != 0){
					$feedbackAanwezig = 'fail';	
					 $sql = 'update tblgroepuserevent SET isAanwezig ="1" where eventid = "'. $eventid . '" AND groepid = "'.$_SESSION['idgroep']. '" AND userid = "'.$_SESSION['userid'].'"';
					 $result  = $db->conn->query($sql);


	            }else{
		            $sql = 'insert into tblgroepuserevent(eventid, groepid, userid, isAanwezig) values ("' . $eventid . '","' . $_SESSION['idgroep'] . '","'  . $_SESSION['userid']. '", "1")';
					$result = $db->conn->query($sql);
